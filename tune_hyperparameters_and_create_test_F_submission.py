...
if __name__ == '__main__':
    ...
    inl_ths = [0.1, 0.2, 0.25, 0.5, 0.75, 1.0]#, 1.5, 2.0]
    ...
    for m_th in match_ths:
        for inl_th in inl_ths:
            ...
            run_str = f'python -utt create_F_submission.py --data_dir {args.data_dir} --conf {args.conf} --match_th {m_th} --inlier_th {inl_th} --method {args.method} --split val --maxiter {args.maxiter}'
            os.system(run_str)
            run_str_eval = f'python -utt eval_F_submission.py --data_dir {args.data_dir} --conf {args.conf} --match_th {m_th} --inlier_th {inl_th} --method {args.method} --split val --maxiter {args.maxiter}'
    ...
    run_str = f'python -utt create_F_submission.py --data_dir {args.data_dir} --conf {args.conf} --match_th {match_good} --inlier_th {inl_good} --method {args.method} --split test --maxiter {args.maxiter}'
    os.system(run_str)
    print ('Done!')
