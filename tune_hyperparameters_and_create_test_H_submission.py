...
if __name__ == '__main__':
    ...
    inl_ths = [0.1, 0.25, 0.5, 0.75, 1.0, 1.5, 2.0]
    match_ths = [0.7, 0.75, 0.8, 0.85, 0.9]
    res = {}
    for m_th in match_ths:
        for inl_th in inl_ths:
            key = f'{inl_th}_{m_th}'
            print (f'inlier threshold = {inl_th}, match threshold={m_th}')
            run_str = f'python -utt create_H_submission.py --data_dir {args.data_dir} --conf {args.conf} --match_th {m_th} --inlier_th {inl_th} --method {args.method} --split val --maxiter {args.maxiter}'
            os.system(run_str)
            run_str_eval = f'python -utt eval_H_submission.py --data_dir {args.data_dir} --conf {args.conf} --match_th {m_th} --inlier_th {inl_th} --method {args.method} --split val --maxiter {args.maxiter}'
            ...
            OUT_DIR = get_output_dir('h', 'val', args.method, params)
